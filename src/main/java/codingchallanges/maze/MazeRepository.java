...
public interface MazeRepository extends JpaRepository<Maze, Long> {
	...
	@Query("SELECT new codingchallanges.maze.MazeData(m.id, m.size, m.coins, m.bumps, m.creationTime, m.finishTime, m.status, m.username) FROM Maze m where m.status = 'WON' ORDER BY m.coins desc, m.bumps asc")
